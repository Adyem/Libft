...
FT_TEST(test_task_scheduler_metrics_flow, "ft_task_scheduler tracks queue and worker metrics")
{
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, delayed_handle.get_error());
    scheduled_count = scheduler_instance.get_scheduled_task_count();
    ...
    blocking_future.get();
    FT_ASSERT_EQ(FT_ER_SUCCESSS, blocking_future.get_error());
    queued_future.get();
    FT_ASSERT_EQ(FT_ER_SUCCESSS, queued_future.get_error());
    usleep(20000);
    queue_size = scheduler_instance.get_queue_size();
    FT_ASSERT_EQ(0, queue_size);
    FT_ASSERT_EQ(FT_ER_SUCCESSS, scheduler_instance.get_error());
