#include "../test_internal.hpp"
#include "../../Modules/Game/game_dialogue_table.hpp"
#include "../../Modules/Game/game_dialogue_line.hpp"
#include "../../Modules/Game/game_dialogue_script.hpp"
#include "../../Modules/Template/shared_ptr.hpp"
#include "../../Modules/Template/vector.hpp"
#include <cstring>
#include "../../Modules/System_utils/test_system_utils_runner.hpp"

#include "../../Modules/Basic/class_nullptr.hpp"
#include "../../Modules/Basic/limits.hpp"
#include "../../Modules/Errno/errno.hpp"
#include "../../Modules/PThread/mutex.hpp"
#include "../../Modules/PThread/recursive_mutex.hpp"
#include "../../Modules/Template/pair.hpp"
#ifndef LIBFT_TEST_BUILD
#endif

static int32_t build_line(ft_sharedptr<game_dialogue_line> &stored, int id, const char *speaker,
        const char *text)
{
    ft_vector<int> next;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, next.initialize());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, next.push_back(id + 1));
    stored = ft_sharedptr<game_dialogue_line>(new (std::nothrow) game_dialogue_line());
    FT_ASSERT(stored.get() != ft_nullptr);
    ft_string speaker_string;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, speaker_string.initialize(speaker));
    ft_string text_string;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, text_string.initialize(text));
    FT_ASSERT_EQ(FT_ERR_SUCCESS, stored->initialize());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, stored->initialize(id,
        speaker_string, text_string, next));
    return (FT_ERR_SUCCESS);
}

FT_TEST(test_dialogue_table_get_lines_accessor_returns_map)
{
    game_dialogue_table table;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.initialize());
    ft_map<int, ft_sharedptr<game_dialogue_line>> &lines = table.get_lines();

    ft_sharedptr<game_dialogue_line> stored_line;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, build_line(stored_line, 3, "npc", "hello"));
    lines.insert(3, stored_line);
    FT_ASSERT_EQ(1, lines.size());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, stored_line->get_error());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.get_error());
    return (1);
}

FT_TEST(test_dialogue_table_get_scripts_accessor_returns_map)
{
    game_dialogue_table table;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.initialize());
    ft_map<int, game_dialogue_script> &scripts = table.get_scripts();

    game_dialogue_script script;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, script.initialize());
    script.set_script_id(5);
    scripts.insert(5, script);
    FT_ASSERT_EQ(1, scripts.size());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, script.get_error());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.get_error());
    return (1);
}

FT_TEST(test_dialogue_table_scripts_map_mutation_does_not_spoil_storage)
{
    game_dialogue_table table;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.initialize());
    ft_map<int, game_dialogue_script> &scripts = table.get_scripts();
    game_dialogue_script script;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, script.initialize());
    script.set_script_id(7);
    ft_string intro;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, intro.initialize("intro"));
    script.set_title(intro);
    ft_vector<ft_sharedptr<game_dialogue_line>> lines;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, lines.initialize());
    ft_sharedptr<game_dialogue_line> line;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, build_line(line, 7, "npc", "reply"));
    FT_ASSERT_EQ(FT_ERR_SUCCESS, lines.push_back(line));
    script.set_lines(lines);
    scripts.insert(7, script);

    game_dialogue_script &stored = scripts.at(7);
    ft_string modified;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, modified.initialize("modified"));
    stored.set_title(modified);
    game_dialogue_script fetched;
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.fetch_script(7, fetched));
    FT_ASSERT_NEQ(0, std::strcmp("modified", script.get_title().c_str()));
    FT_ASSERT_STR_EQ("modified", stored.get_title().c_str());
    FT_ASSERT_STR_EQ("modified", fetched.get_title().c_str());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, table.get_error());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, fetched.get_error());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, script.get_error());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, stored.get_error());
    FT_ASSERT_EQ(FT_ERR_SUCCESS, lines[0]->get_error());
    return (1);
}
