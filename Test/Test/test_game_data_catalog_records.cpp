...
{
    ft_item_definition original(5, 3, 10, 2, 4, 6, 7);
    ft_item_definition copy(original);
    ft_item_definition moved(ft_move(original));
...
FT_TEST(test_recipe_blueprint_copy_move, "copy and move recipe blueprints")
{
    ...
    blueprint = ft_recipe_blueprint(12, 30, ingredients);

    copy = ft_recipe_blueprint(blueprint);
    ...
    moved = ft_recipe_blueprint(ft_move(blueprint));
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, moved.get_error());
    FT_ASSERT_EQ(FT_ER_SUCCESSS, blueprint.get_error());
    return (1);
}

FT_TEST(test_loadout_entry_copy_move, "copy and move loadout entries")
{
    ft_loadout_entry entry(2, 15, 3);
    ft_loadout_entry copied(entry);
    ft_loadout_entry moved(ft_move(entry));
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, moved.get_error());
    FT_ASSERT_EQ(FT_ER_SUCCESSS, entry.get_error());
    return (1);
}
...
FT_TEST(test_loadout_blueprint_copy_move, "copy and move loadout blueprints")
{
    ...
    entries.push_back(entry);
    blueprint = ft_loadout_blueprint(20, entries);

    copy = ft_loadout_blueprint(blueprint);
    ...
    moved = ft_loadout_blueprint(ft_move(blueprint));
