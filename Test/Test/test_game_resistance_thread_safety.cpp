...
static void *game_resistance_read_task(void *argument)
{
    ...
    while (index < arguments->iterations)
    {
        ...
        if (arguments->resistance_pointer->get_error() != FT_ERR_SUCCESSS)
        {
            (void)percent_value;
    ...
    "ft_resistance serializes concurrent updates and reads")
{
    ...
    while (index < 128 && test_failed == 0)
    {
        ...
        if (test_failed == 0)
        {
            ...
            if (primary_resistance.get_error() != FT_ERR_SUCCESSS)
            {
                test_failed = 1;
                failure_expression = "primary_resistance.get_error() == FT_ERR_SUCCESSS";
                failure_line = __LINE__;
            }
        }
        index += 1;
