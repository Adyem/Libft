...
static bool http2_test_receive_single_frame(int socket_fd, http2_frame &out_frame)
{
    ...
    if (payload_buffer.initialize() != FT_ERR_SUCCESS)
        return (false);
    ...
    while (remaining_length > 0)
    {
        ...
        if (received_bytes <= 0)
            return (false);
        if (payload_buffer.append(chunk_buffer,
                static_cast<size_t>(received_bytes)) != FT_ERR_SUCCESS)
            return (false);
...
static bool http2_test_drain_client_headers(int socket_fd)
{
    ...
    while (!headers_complete)
    {
        ...
        if (received_bytes <= 0)
            return (false);
        if (buffer.append(receive_buffer,
                static_cast<size_t>(received_bytes)) != FT_ERR_SUCCESS)
            return (false);
        size_t parse_offset;
        ...
        if (parse_offset > 0)
        {
            if (buffer.erase(0, parse_offset) != FT_ERR_SUCCESS)
                return (false);
        }
    }
    return (true);
...
static int http2_test_server(http2_test_server_state *state)
{
    ...
    if (client_frame.initialize() != FT_ERR_SUCCESS)
    {
        state->start_error.store(FT_ERR_NO_MEMORY,
            std::memory_order_relaxed);
        state->ready.store(true, std::memory_order_release);
        return (FT_ERR_NO_MEMORY);
    }
    if (server_frame.initialize() != FT_ERR_SUCCESS)
    {
        state->start_error.store(FT_ERR_NO_MEMORY,
            std::memory_order_relaxed);
        state->ready.store(true, std::memory_order_release);
    ...
    {
        state->start_error.store(config_error, std::memory_order_relaxed);
        state->ready.store(true, std::memory_order_release);
        return (config_error);
    }
    ...
    {
        if (encode_error == FT_ERR_SUCCESS)
    ...
    {
        state->result.store(FT_ERR_NO_MEMORY, std::memory_order_relaxed);
        nw_close(client_fd);
        return (FT_ERR_NO_MEMORY);
    }
    ...
    if (response_headers.get_error() != FT_ERR_SUCCESS)
    {
        state->result.store(response_headers.get_error(),
            std::memory_order_relaxed);
        nw_close(client_fd);
        return (response_headers.get_error());
    }
...
static ft_bool http2_test_capture_reset(http2_stream_capture &capture)
{
    if (capture.headers.is_initialised() != FT_CLASS_STATE_INITIALISED)
    {
        if (capture.headers.initialize() != FT_ERR_SUCCESS)
            return (FT_FALSE);
    }
    if (capture.body.is_initialised() != FT_CLASS_STATE_INITIALISED)
    {
        if (capture.body.initialize() != FT_ERR_SUCCESS)
            return (FT_FALSE);
    }
...
static ft_bool http2_test_body_callback(const char *chunk_data, size_t chunk_size,
    ft_bool is_final, void *user_data)
{
    http2_stream_capture *capture;
    size_t index;
...
FT_TEST(test_api_http_plain_http2_streaming_success)
{
    ...
    FT_ASSERT(http2_test_capture_reset(capture));
