        ...
        int iterations, int *failure_flag)
{
    ...
    while (index < iterations)
    {
        ...
        if (reputation->get_error() != ER_SUCCESS)
        {
            *failure_flag = 1;
            return ;
        }
        ...
        "ft_reputation assignment avoids deadlock and preserves mutex ordering")
{
    ...
    FT_ASSERT_EQ(ER_SUCCESS, left_reputation.get_error());
    ...
    FT_ASSERT_EQ(ER_SUCCESS, right_reputation.get_error());
