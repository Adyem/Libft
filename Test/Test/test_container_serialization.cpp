    ...
    "ft_vector JSON serialization round trips integral values")
{
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, numbers.get_error());
    numbers.push_back(-4);
    FT_ASSERT_EQ(FT_ER_SUCCESSS, numbers.get_error());
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, parsed.get_error());
...
static int ft_entry_serializer(const ft_serialized_entry &entry, ft_string &output) noexcept
{
    ...
    if (separator.get_error() != FT_ER_SUCCESSS)
        return (-1);
    output += separator;
    if (output.get_error() != FT_ER_SUCCESSS)
        return (-1);
    ...
    if (quantity_string.get_error() != FT_ER_SUCCESSS)
        return (-1);
...
static int ft_entry_deserializer(const char *value_string, ft_serialized_entry &entry) noexcept
{
    ...
    if (name_buffer.get_error() != FT_ER_SUCCESSS)
        return (-1);
    entry.name = name_buffer;
    if (entry.name.get_error() != FT_ER_SUCCESSS)
        return (-1);
    ...
    "ft_vector JSON serialization accepts custom serializers")
{
    ...
    first.name = "apples";
    FT_ASSERT_EQ(FT_ER_SUCCESSS, first.name.get_error());
    first.quantity = 5;
    second.name = "oranges";
    FT_ASSERT_EQ(FT_ER_SUCCESSS, second.name.get_error());
    second.quantity = 9;
    ft_vector<ft_serialized_entry> entries;

    entries.push_back(first);
    FT_ASSERT_EQ(FT_ER_SUCCESSS, entries.get_error());
