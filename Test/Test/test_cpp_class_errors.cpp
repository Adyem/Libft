        ...
        "DataBuffer copy and move keep error codes but reset errno to success")
{
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, move_constructor_source.get_error());
    FT_ASSERT_EQ(FT_ER_SUCCESSS, ft_errno);
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, move_assignment_source.get_error());
    FT_ASSERT_EQ(FT_ER_SUCCESSS, ft_errno);
    return (1);
}
...
FT_TEST(test_ft_file_error_resets, "ft_file resets error state after success")
{
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, file.get_error());
    FT_ASSERT_EQ(FT_ER_SUCCESSS, ft_errno);
    file.close();
    ::unlink(filename);
    return (1);
...
FT_TEST(test_ft_fd_istream_error_resets, "ft_fd_istream sets su_read errno and clears after success")
{
    ...
    FT_ASSERT(failure_error != FT_ER_SUCCESSS);
