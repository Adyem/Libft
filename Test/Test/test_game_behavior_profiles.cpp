...
FT_TEST(test_behavior_register_and_fetch, "register and fetch behavior profile")
{
    ...
    FT_ASSERT_EQ(ER_SUCCESS, table.register_profile(profile));
    FT_ASSERT_EQ(ER_SUCCESS, table.fetch_profile(5, fetched));
...
FT_TEST(test_behavior_profile_isolation, "fetched copies remain isolated")
{
    ...
    FT_ASSERT_EQ(ER_SUCCESS, table.register_profile(profile));
    FT_ASSERT_EQ(ER_SUCCESS, table.fetch_profile(20, first_fetch));
    first_fetch.get_actions()[0].set_weight(2.5);
    FT_ASSERT_EQ(ER_SUCCESS, table.fetch_profile(20, second_fetch));
