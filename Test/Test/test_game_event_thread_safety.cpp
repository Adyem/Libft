...
FT_TEST(test_game_event_add_duration_thread_safe, "ft_event::add_duration remains consistent under concurrent increments")
{
    ...
    while (index < 4)
    {
        ...
        arguments[index].result_code = FT_ER_SUCCESSS;
        if (test_failed == 0)
        {
    ...
    while (index < created_thread_count)
    {
        ...
        if (join_result == 0)
        {
            if (arguments[index].result_code != FT_ER_SUCCESSS && test_failed == 0)
            {
                test_failed = 1;
                failure_expression = "arguments[index].result_code == FT_ER_SUCCESSS";
                failure_line = __LINE__;
            }
        }
        index++;
...
FT_TEST(test_game_event_assignment_thread_safe, "ft_event copy assignment synchronizes shared access")
{
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, source_event.get_error());
    ...
    FT_ASSERT_EQ(FT_ER_SUCCESSS, destination_event.get_error());
    created_thread_count = 0;
    test_failed = 0;
    failure_expression = ft_nullptr;
    failure_line = 0;
