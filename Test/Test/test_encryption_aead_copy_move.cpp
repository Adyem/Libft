        ...
        size_t &output_length)
{
    ...
    output_length = 0;
    FT_ASSERT_EQ(FT_ERR_SUCCESSS, context.update(plaintext, 6, ciphertext, output_length));
    FT_ASSERT_EQ(static_cast<size_t>(6), output_length);
    FT_ASSERT_EQ(FT_ERR_SUCCESSS, context.finalize(authentication_tag, 16));
        ...
        "encryption_aead_context move constructor initializes an independent mutex")
{
    ...
    fill_sequence(iv, 12);
    FT_ASSERT_EQ(FT_ERR_SUCCESSS, source.initialize_encrypt(key, 16, iv, 12));
