...
FT_TEST(test_world_register_and_fetch, "register and fetch regions and worlds")
{
    ...
    FT_ASSERT_EQ(ER_SUCCESS, registry.fetch_region(7, fetched_region));
    FT_ASSERT_EQ(ft_string("meadow"), fetched_region.get_name());
    FT_ASSERT_EQ(ER_SUCCESS, registry.fetch_world(1, fetched_world));
...
FT_TEST(test_world_isolation, "fetched values remain isolated")
{
    ...
    FT_ASSERT_EQ(ER_SUCCESS, registry.register_world(world));
    FT_ASSERT_EQ(ER_SUCCESS, registry.fetch_region(11, region));
    region.set_name(ft_string("changed"));
    FT_ASSERT_EQ(ER_SUCCESS, registry.fetch_region(11, fetched_again));
    FT_ASSERT_EQ(ft_string("volcano"), fetched_again.get_name());
    FT_ASSERT_EQ(ER_SUCCESS, registry.fetch_world(2, world_again));
    world_again.get_region_ids()[0] = 99;
    FT_ASSERT_EQ(ER_SUCCESS, registry.fetch_world(2, fetched_world));
