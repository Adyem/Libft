    ...
    "Game deterministic simulation across combat, crafting, quests, and events")
{
    ...
    FT_ASSERT_EQ(world_instance.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(enemy_character.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(oak_plank.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(iron_ingot.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(hero_inventory.add_item(oak_plank), FT_ER_SUCCESSS);
    FT_ASSERT_EQ(hero_inventory.add_item(iron_ingot), FT_ER_SUCCESSS);
    FT_ASSERT_EQ(hero_inventory.get_error(), FT_ER_SUCCESSS);

    ft_crafting crafting_system;
    FT_ASSERT_EQ(crafting_system.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(sword_recipe.get_error(), FT_ER_SUCCESSS);
    sword_recipe.push_back(iron_requirement);
    FT_ASSERT_EQ(sword_recipe.get_error(), FT_ER_SUCCESSS);

    int const crafted_sword_recipe_id = 501;
    FT_ASSERT_EQ(crafting_system.register_recipe(crafted_sword_recipe_id, ft_move(sword_recipe)), FT_ER_SUCCESSS);
    FT_ASSERT_EQ(crafting_system.get_error(), FT_ER_SUCCESSS);

    ft_sharedptr<ft_item> crafted_sword(new (std::nothrow) ft_item());
    FT_ASSERT(crafted_sword.get() != ft_nullptr);
    FT_ASSERT_EQ(crafted_sword.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(crafting_quest.get_error(), FT_ER_SUCCESSS);
    hero_character.get_quests().insert(crafting_quest.get_id(), crafting_quest);
    FT_ASSERT_EQ(hero_character.get_quests().get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(crafting_event.get_error(), FT_ER_SUCCESSS);
    ...
    crafting_event->set_callback(ft_function<void(ft_world&, ft_event&)>([&](ft_world &world_reference, ft_event &event_reference)
    {
        ...
        if (crafting_result_code == FT_ER_SUCCESSS)
        {
    ...
    FT_ASSERT_EQ(combat_event.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(completion_event.get_error(), FT_ER_SUCCESSS);
    ...
    FT_ASSERT_EQ(world_instance->get_error(), FT_ER_SUCCESSS);
    FT_ASSERT_EQ(crafting_result_code, FT_ER_SUCCESSS);
    ...
    world_instance->update_events(world_instance, 1);
    FT_ASSERT_EQ(world_instance->get_error(), FT_ER_SUCCESSS);
    FT_ASSERT_EQ(combat_result_code, FT_ER_SUCCESSS);
    FT_ASSERT_EQ(enemy_character.get_hit_points(), 22);

    world_instance->update_events(world_instance, 1);
    FT_ASSERT_EQ(world_instance->get_error(), FT_ER_SUCCESSS);
    FT_ASSERT_EQ(completion_result_code, FT_ER_SUCCESSS);
