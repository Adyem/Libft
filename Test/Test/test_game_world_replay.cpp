    ...
    "ft_world_replay_session captures snapshots, restores, and replays deterministically")
{
    ...
    FT_ASSERT_EQ(world_pointer.get_error(), ER_SUCCESS);
    FT_ASSERT_EQ(session.get_error(), ER_SUCCESS);
    ...
    FT_ASSERT_EQ(healing_potion->get_error(), ER_SUCCESS);
    FT_ASSERT_EQ(hero_inventory.add_item(healing_potion), ER_SUCCESS);
    FT_ASSERT_EQ(hero_inventory.get_error(), ER_SUCCESS);
    ...
    FT_ASSERT_EQ(damage_event->get_error(), ER_SUCCESS);
    ...
    FT_ASSERT_EQ(ER_SUCCESS, capture_result);
    FT_ASSERT_EQ(ER_SUCCESS, session.get_error());
    ...
    FT_ASSERT_EQ(ER_SUCCESS, restore_result);
    FT_ASSERT_EQ(ER_SUCCESS, session.get_error());
    ...
    FT_ASSERT_EQ(ER_SUCCESS, replay_result);
    FT_ASSERT_EQ(ER_SUCCESS, session.get_error());
    FT_ASSERT_EQ(hero_character.get_hit_points(), 15);
    FT_ASSERT_EQ(hero_inventory.count_item(301), 1);
    FT_ASSERT_EQ(world_pointer->get_event_scheduler()->size(), static_cast<size_t>(0));
    ...
    "ft_world_replay_session imports, exports, and clears snapshots while propagating errors")
{
    ...
    FT_ASSERT_EQ(session.capture_snapshot(*world_pointer, hero_character, hero_inventory), ER_SUCCESS);
    FT_ASSERT_EQ(session.get_error(), ER_SUCCESS);

    FT_ASSERT_EQ(session.export_snapshot(exported_snapshot), ER_SUCCESS);
    FT_ASSERT_EQ(session.get_error(), ER_SUCCESS);
    ...
    FT_ASSERT_EQ(session.export_snapshot(cleared_snapshot), ER_SUCCESS);
    FT_ASSERT_EQ(session.get_error(), ER_SUCCESS);
    FT_ASSERT(cleared_snapshot.empty());

    FT_ASSERT_EQ(imported_session.import_snapshot(exported_snapshot), ER_SUCCESS);
    FT_ASSERT_EQ(imported_session.get_error(), ER_SUCCESS);

    hero_character.set_hit_points(10);
    FT_ASSERT_EQ(hero_character.get_error(), ER_SUCCESS);

    FT_ASSERT_EQ(imported_session.restore_snapshot(world_pointer, hero_character, hero_inventory), ER_SUCCESS);
    FT_ASSERT_EQ(imported_session.get_error(), ER_SUCCESS);
