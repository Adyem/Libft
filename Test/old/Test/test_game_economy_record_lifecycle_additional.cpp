...
FT_TEST(test_price_lifecycle_get_item_id_uninitialised_aborts, "price definition lifecycle aborts for get_item_id while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(price_get_item_id));
    return (1);
}

FT_TEST(test_price_lifecycle_get_rarity_uninitialised_aborts, "price definition lifecycle aborts for get_rarity while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(price_get_rarity));
    return (1);
}

FT_TEST(test_price_lifecycle_get_base_value_uninitialised_aborts, "price definition lifecycle aborts for get_base_value while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(price_get_base_value));
    return (1);
}

FT_TEST(test_price_lifecycle_set_item_id_uninitialised_aborts, "price definition lifecycle aborts for set_item_id while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(price_set_item_id));
    return (1);
}

FT_TEST(test_price_lifecycle_set_rarity_uninitialised_aborts, "price definition lifecycle aborts for set_rarity while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(price_set_rarity));
    return (1);
}

FT_TEST(test_rarity_lifecycle_get_rarity_uninitialised_aborts, "rarity band lifecycle aborts for get_rarity while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(rarity_get_rarity));
    return (1);
}

FT_TEST(test_rarity_lifecycle_get_value_multiplier_uninitialised_aborts, "rarity band lifecycle aborts for get_value_multiplier while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(rarity_get_value_multiplier));
    return (1);
}

FT_TEST(test_rarity_lifecycle_set_rarity_uninitialised_aborts, "rarity band lifecycle aborts for set_rarity while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(rarity_set_rarity));
    return (1);
}

FT_TEST(test_rarity_lifecycle_set_value_multiplier_uninitialised_aborts, "rarity band lifecycle aborts for set_value_multiplier while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(rarity_set_value_multiplier));
    return (1);
}

FT_TEST(test_rarity_lifecycle_is_thread_safe_uninitialised_aborts, "rarity band lifecycle aborts for is_thread_safe while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(rarity_is_thread_safe));
    return (1);
}

FT_TEST(test_vendor_lifecycle_get_vendor_id_uninitialised_aborts, "vendor profile lifecycle aborts for get_vendor_id while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(vendor_get_vendor_id));
    return (1);
}

FT_TEST(test_vendor_lifecycle_get_buy_markup_uninitialised_aborts, "vendor profile lifecycle aborts for get_buy_markup while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(vendor_get_buy_markup));
    return (1);
}

FT_TEST(test_vendor_lifecycle_set_vendor_id_uninitialised_aborts, "vendor profile lifecycle aborts for set_vendor_id while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(vendor_set_vendor_id));
    return (1);
}

FT_TEST(test_vendor_lifecycle_set_buy_markup_uninitialised_aborts, "vendor profile lifecycle aborts for set_buy_markup while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(vendor_set_buy_markup));
    return (1);
}

FT_TEST(test_vendor_lifecycle_enable_thread_safety_uninitialised_aborts, "vendor profile lifecycle aborts for enable_thread_safety while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(vendor_enable_thread_safety));
    return (1);
}

FT_TEST(test_currency_lifecycle_get_currency_id_uninitialised_aborts, "currency rate lifecycle aborts for get_currency_id while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(currency_get_currency_id));
    return (1);
}

FT_TEST(test_currency_lifecycle_get_rate_to_base_uninitialised_aborts, "currency rate lifecycle aborts for get_rate_to_base while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(currency_get_rate_to_base));
    return (1);
}

FT_TEST(test_currency_lifecycle_get_display_precision_uninitialised_aborts, "currency rate lifecycle aborts for get_display_precision while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(currency_get_display_precision));
    return (1);
}

FT_TEST(test_currency_lifecycle_set_currency_id_uninitialised_aborts, "currency rate lifecycle aborts for set_currency_id while uninitialised")
{
    FT_ASSERT_EQ(1, expect_sigabrt_on_uninitialised_object(currency_set_currency_id));
    return (1);
}

FT_TEST(test_currency_lifecycle_set_rate_to_base_uninitialised_aborts, "currency rate lifecycle aborts for set_rate_to_base while uninitialised")
{
