...
- Replaced the manual `start_data` reference counting with a shared launch capsule owned by both the parent and worker threads so the storage is not returned to CMA until `join()` or `detach()` completes.【F:Threading/thread.hpp†L17-L82】【F:PThread/pthread_thread.cpp†L1-L116】
