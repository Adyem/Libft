...
static int report_atoi_result(int error_code, int return_value)
{
...
int    ft_atoi(const char *string)
{
    ...
    if (string == ft_nullptr)
        return (report_atoi_result(FT_ERR_INVALID_ARGUMENT, 0));
                ...
                && (string[index] <= '9')))
    {
        ...
        if (sign == 1)
        {
            if (result > positive_limit / 10)
                return (report_atoi_result(FT_ERR_OUT_OF_RANGE, FT_INT_MAX));
            if (result == positive_limit / 10
        ...
        else
        {
            if (result > negative_limit / 10)
                return (report_atoi_result(FT_ERR_OUT_OF_RANGE, FT_INT_MIN));
            if (result == negative_limit / 10
    ...
    if (error_code != FT_ERR_SUCCESSS)
    {
        if (sign == -1)
        {
            ...
            return (report_atoi_result(error_code,
                static_cast<int>(signed_result)));
        }
        return (report_atoi_result(error_code, static_cast<int>(result)));
    }
    if (sign == -1)
    {
        long long signed_result = -static_cast<long long>(result);

        return (report_atoi_result(FT_ERR_SUCCESSS,
            static_cast<int>(signed_result)));
    }
    return (report_atoi_result(FT_ERR_SUCCESSS, static_cast<int>(result)));
}
