...
int deserialize_inventory(ft_inventory &inventory, json_group *group)
{
    ...
    while (item_index < item_count)
    {
        ...
        ft_item item_temp;
        if (build_item_from_group(item_temp, group, item_prefix) != FT_ERR_SUCCESSS)
        {
        ...
        if (inventory.add_item(item) != FT_ERR_SUCCESSS)
        {
...
int deserialize_equipment(ft_character &character, json_group *group)
{
    ...
    if (present && ft_atoi(present->value) == 1)
    {
        ...
        if (build_item_from_group(item_temp, group, "head") != FT_ERR_SUCCESSS)
            return (FT_ERR_GAME_GENERAL_ERROR);
        ft_sharedptr<ft_item> item(new ft_item(item_temp));
        if (character.equip_item(EQUIP_HEAD, item) != FT_ERR_SUCCESSS)
    ...
    if (present && ft_atoi(present->value) == 1)
    {
        ...
        if (build_item_from_group(item_temp, group, "chest") != FT_ERR_SUCCESSS)
            return (FT_ERR_GAME_GENERAL_ERROR);
        ft_sharedptr<ft_item> item(new ft_item(item_temp));
        if (character.equip_item(EQUIP_CHEST, item) != FT_ERR_SUCCESSS)
            return (character.get_error());
    }
    else
    ...
    if (present && ft_atoi(present->value) == 1)
    {
        ...
        if (build_item_from_group(item_temp, group, "weapon") != FT_ERR_SUCCESSS)
            return (FT_ERR_GAME_GENERAL_ERROR);
        ft_sharedptr<ft_item> item(new ft_item(item_temp));
        if (character.equip_item(EQUIP_WEAPON, item) != FT_ERR_SUCCESSS)
            return (character.get_error());
    }
    else
...
int deserialize_quest(ft_quest &quest, json_group *group)
{
    ...
    if (count_item)
    {
        ...
        while (reward_index < reward_count)
        {
            ...
            ft_item reward_temp;
            if (build_item_from_group(reward_temp, group, prefix) != FT_ERR_SUCCESSS)
                return (FT_ERR_GAME_GENERAL_ERROR);
