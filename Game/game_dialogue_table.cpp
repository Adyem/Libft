...
void ft_dialogue_table::record_operation_error_unlocked(int error_code)
{
    ...
    ft_operation_error_stack_push(ft_dialogue_table::_operation_errors,
...
static void game_dialogue_table_unlock_set_errno(ft_unique_lock<pt_mutex> &guard, int target_error)
{
    ...
    {
        ft_dialogue_table::record_operation_error_unlocked(unlock_error);
        return ;
    }
    ft_dialogue_table::record_operation_error_unlocked(target_error);
    return ;
}
...
int ft_dialogue_table::clone_from(const ft_dialogue_table &other) noexcept
{
    ...
    ft_dialogue_table::record_operation_error_unlocked(FT_ERR_SUCCESS);
    return (FT_ERR_SUCCESS);
}

ft_dialogue_table::ft_dialogue_table() noexcept
    : _lines(), _scripts(), _error_code(FT_ERR_SUCCESS)
{
    return ;
}
    ...
    : _lines(), _scripts(), _error_code(FT_ERR_SUCCESS)
{
    ...
    if (this->clone_from(other) != FT_ERR_SUCCESS)
    {
        return ;
    }
    this->_error_code = FT_ERR_SUCCESS;
    this->set_error(this->_error_code);
    game_dialogue_table_unlock_set_errno(other_guard, FT_ERR_SUCCESS);
    return ;
}
...
ft_dialogue_table &ft_dialogue_table::operator=(const ft_dialogue_table &other) noexcept
{
    ...
    this->set_error(this->_error_code);
    game_dialogue_table_unlock_set_errno(other_guard, FT_ERR_SUCCESS);
    game_dialogue_table_unlock_set_errno(this_guard, FT_ERR_SUCCESS);
    return (*this);
}
...
int ft_dialogue_table::register_script(const ft_dialogue_script &script) noexcept
{
    ...
    if (script.get_error() != FT_ERR_SUCCESS)
    {
...
int ft_dialogue_table::get_error() const noexcept
{
    ...
    if (guard.get_error() != FT_ERR_SUCCESS)
    {
        ft_dialogue_table::record_operation_error_unlocked(guard.get_error());
        return (guard.get_error());
    }
...
void ft_dialogue_table::set_error(int error_code) const noexcept
{
    ...
    ft_dialogue_table::record_operation_error_unlocked(error_code);
    return ;
}
