        ...
        int entry_errno, int result_errno)
{
    ...
    if (unlock_errno != FT_ER_SUCCESSS)
    {
...
ft_event::ft_event() noexcept
    : _id(0), _duration(0), _modifier1(0), _modifier2(0), _modifier3(0),
      _modifier4(0), _callback(), _error(FT_ER_SUCCESSS), _mutex()
{
    if (this->_callback.get_error() != FT_ER_SUCCESSS)
    {
      ...
      _modifier4(0), _callback(), _error(FT_ER_SUCCESSS), _mutex()
{
    ...
    if (callback_error != FT_ER_SUCCESSS)
    {
...
int ft_event::get_id() const noexcept
{
    ...
    identifier = this->_id;
    const_cast<ft_event *>(this)->set_error(FT_ER_SUCCESSS);
    restore_error = game_event_restore_errno(guard, entry_errno, FT_ER_SUCCESSS);
    if (restore_error != FT_ER_SUCCESSS)
    {
        const_cast<ft_event *>(this)->set_error(restore_error);
        return (0);
    }
