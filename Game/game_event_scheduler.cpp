        ...
        t_high_resolution_time_point start_time) noexcept
{
    ...
    if (this->_ready_cache.get_error() != FT_ER_SUCCESSS)
    {
        ...
        const ft_sharedptr<ft_event> &right) const noexcept
{
    ...
    left_pointer->set_error(FT_ER_SUCCESSS);
    right_pointer->set_error(FT_ER_SUCCESSS);
    if (left_guard.owns_lock())
        left_guard.unlock();
    if (right_guard.owns_lock())
        right_guard.unlock();
...
ft_event_scheduler::ft_event_scheduler() noexcept
    : _events(), _error_code(FT_ER_SUCCESSS), _mutex(), _profiling_enabled(false), _profile(), _ready_cache()
{
    event_scheduler_profile_reset_struct(this->_profile);
    if (this->_events.get_error() != FT_ER_SUCCESSS)
    ...
    : _events(), _error_code(FT_ER_SUCCESSS), _mutex(), _profiling_enabled(false), _profile(), _ready_cache()
{
    ...
    if (other.get_error() != FT_ER_SUCCESSS)
    {
...
void ft_event_scheduler::cancel_event(int id) noexcept
{
    ...
    while (!this->_events.empty())
    {
        ...
        if (current_event->get_id() != id)
        {
            ...
            if (temporary_queue.get_error() != FT_ER_SUCCESSS)
            {
...
void ft_event_scheduler::reschedule_event(int id, int new_duration) noexcept
{
    ...
    while (!this->_events.empty())
    {
        ...
        if (current_event->get_id() == id)
        {
            ...
            if (current_event->get_error() != FT_ER_SUCCESSS)
            {
        ...
        ft_string *log_buffer) noexcept
{
    ...
    ready_cache.clear();
    if (ready_cache.get_error() != FT_ER_SUCCESSS)
    {
    ...
    while (!temporary_queue.empty())
    {
        ...
        if (temporary_event.get_error() != FT_ER_SUCCESSS)
        {
    ...
    while (ready_index < ready_count)
    {
        ...
        if (processed_error != FT_ER_SUCCESSS)
        {
            ...
            if (error_guard.get_error() == FT_ER_SUCCESSS)
            {
...
void ft_event_scheduler::dump_events(ft_vector<ft_sharedptr<ft_event> > &out) const noexcept
{
    ...
    while (!this->_events.empty())
    {
        ...
        if (out.get_error() != FT_ER_SUCCESSS)
        {
...
json_group *serialize_event_scheduler(const ft_sharedptr<ft_event_scheduler> &scheduler)
{
    ...
    while (event_index < event_count)
    {
        ...
        if (add_item_field(group, key_id, events[event_index]->get_id()) != FT_ER_SUCCESSS ||
            add_item_field(group, key_duration, events[event_index]->get_duration()) != FT_ER_SUCCESSS)
            return (ft_nullptr);
        event_index++;
    }
    ft_errno = FT_ER_SUCCESSS;
