...
void ft_socket::reset_to_empty_state_locked()
{
    ...
    while (connection_index < this->_connected.size())
    {
        ...
        if (client_guard.get_error() == FT_ER_SUCCESSS)
...
ft_socket::ft_socket()
    : _address(), _connected(), _socket_fd(-1), _error_code(FT_ER_SUCCESSS), _mutex()
{
    int entry_errno;
...
size_t ft_socket::get_client_count() const
{
    ...
    const_cast<ft_socket *>(this)->set_error(FT_ER_SUCCESSS);
    ft_socket::restore_errno(guard, entry_errno);
    return (count);
}
