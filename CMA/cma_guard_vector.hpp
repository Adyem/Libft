...
cma_guard_vector<t_element>::cma_guard_vector(size_t initial_capacity)
    : _data(ft_nullptr), _size(0), _capacity(0), _error_code(FT_ERR_SUCCESSS)
{
...
cma_guard_vector<t_element> &cma_guard_vector<t_element>::operator=(const cma_guard_vector<t_element> &other)
{
    ...
    if (temp.get_error() != FT_ERR_SUCCESSS && other._size > 0)
    {
    ...
    temp._size = 0;
    temp._capacity = 0;
    temp._error_code = FT_ERR_SUCCESSS;
    this->set_error(FT_ERR_SUCCESSS);
    return (*this);
}
...
{
    const_cast<cma_guard_vector<t_element> *>(this)->set_error(FT_ERR_SUCCESSS);
    return (this->_size);
}
