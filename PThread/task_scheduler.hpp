...
static int pt_mutex_pop_error(pt_mutex &mutex)
{
    int error_value;
...
static int pt_mutex_pop_error(const pt_mutex *mutex)
{
    ...
    return (pt_mutex_pop_error(*mutex));
}
      ...
      _thread_safe_enabled(false)
{
    ...
    {
        this->operation_error_push(this->_condition.get_error());
        return ;
    }
...
int ft_blocking_queue<ElementType>::lock_internal(bool *lock_acquired) const
{
    ...
    int state_error = pt_mutex_pop_error(this->_state_mutex);
...
int ft_blocking_queue<ElementType>::enable_thread_safety()
{
    ...
    mutex_error = pt_mutex_pop_error(state_mutex);
...
void ft_blocking_queue<ElementType>::push(ElementType &&value)
{
    ...
    mutex_error = pt_mutex_pop_error(this->_mutex);
    if (mutex_error != FT_ERR_SUCCESSS)
    {
        this->operation_error_push(mutex_error);
    ...
    if (this->_storage.get_error() != FT_ERR_SUCCESSS)
    {
        ...
        this->operation_error_push(this->_storage.get_error());
        this->unlock_internal(state_lock_acquired);
        return ;
    }
...
bool ft_blocking_queue<ElementType>::pop(ElementType &result)
{
    ...
    if (is_empty)
    {
        ...
        this->operation_error_push(FT_ERR_EMPTY);
        this->unlock_internal(state_lock_acquired);
        return (false);
    }
    ...
    -> ft_future<typename std::invoke_result<FunctionType, Args...>::type>
{
    ...
    {
        this->operation_error_push(promise_shared.get_error());
        ft_future<return_type> empty_future;
        return (empty_future);
    }
    ...
    {
        this->operation_error_push(future_value.get_error());
        return (future_value);
    }
    ...
    if (wrapper.get_error() != FT_ERR_SUCCESSS)
    {
        this->operation_error_push(wrapper.get_error());
        task_body();
        return (future_value);
    }
            ...
            ft_scheduled_task_handle>
{
    ...
    {
        this->operation_error_push(state_shared.get_error());
        raw_task_body();
        return (result_pair);
    }
    ...
    {
        this->operation_error_push(handle_instance.operation_error_last_error());
        return (result_pair);
    }
    ...
    this->_scheduled_mutex.lock(THREAD_ID);
    int scheduled_mutex_error;

    scheduled_mutex_error = pt_mutex_pop_error(this->_scheduled_mutex);
        ...
        FunctionType function, Args... args)
{
    ...
    {
        this->operation_error_push(handle_result.operation_error_last_error());
        return (handle_result);
    }
    task_entry._state = state_shared;
