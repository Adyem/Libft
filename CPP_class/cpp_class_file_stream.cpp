...
static int ft_file_allocate_transfer_buffer(char **buffer_pointer,
...
int ft_file_stream_copy(ft_file &source_file, ft_file &destination_file,
                            size_t buffer_size) noexcept
{
    ...
    while (true)
    {
        ...
        while (write_offset < static_cast<size_t>(bytes_read))
        {
            bytes_written = destination_file.write_buffer(transfer_buffer
                    + write_offset, static_cast<size_t>(bytes_read) - write_offset);
            if (bytes_written < 0)
            {
                ft_file_release_transfer_buffer(transfer_buffer);
                return (-1);
