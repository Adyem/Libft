    ...
    const s_redaction_rule &rule)
{
    ...
    if (rule.pattern.get_error() != FT_ER_SUCCESSS)
        return (rule.pattern.get_error());
    if (rule.replacement.get_error() != FT_ER_SUCCESSS)
    ...
    const char *replacement)
{
    ...
    if (rule.pattern.get_error() != FT_ER_SUCCESSS
        || rule.replacement.get_error() != FT_ER_SUCCESSS)
    {
    ...
    if (g_redaction_rules.get_error() != FT_ER_SUCCESSS)
    {
    ...
    const ft_vector<s_redaction_rule> &rules)
{
    ...
    if (rules.get_error() != FT_ER_SUCCESSS)
    {
