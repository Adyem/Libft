    ...
    const s_redaction_rule &rule)
{
    ...
    if (rule.pattern.get_error() != ER_SUCCESS)
        return (rule.pattern.get_error());
    if (rule.replacement.get_error() != ER_SUCCESS)
    ...
    const char *replacement)
{
    ...
    if (rule.pattern.get_error() != ER_SUCCESS
        || rule.replacement.get_error() != ER_SUCCESS)
    {
    ...
    if (g_redaction_rules.get_error() != ER_SUCCESS)
    {
