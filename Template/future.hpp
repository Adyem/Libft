...
template <typename ValueType>
int32_t ft_future<ValueType>::last_error_code() const
{
    return (FT_ERR_SUCCESS);
}
...
int32_t ft_future<ValueType>::initialize(const ft_future<ValueType> &other)
{
    ...
    int32_t this_lock_result = this->lock_internal(&this_lock_acquired);
    if (this_lock_result != FT_ERR_SUCCESS)
    {
        return (this_lock_result);
    }
    other_lock_acquired = FT_FALSE;
    int32_t other_lock_result = other.lock_internal(&other_lock_acquired);
...
inline int32_t ft_future<void>::last_error_code() const
{
    return (FT_ERR_SUCCESS);
}
